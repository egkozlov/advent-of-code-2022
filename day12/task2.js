...
for (let i = 0; i < pointsMatrix.length; i++) {
  for (let j = 0; j < pointsMatrix[0].length; j++) {
    ...
    if (currentPoint === 'S' || currentPoint === 'a') {
      startingPoints.push(currentPointNodeKey);
    }
    if (currentPoint === 'E') {
...
const allPosibleRoutes = startingPoints.map((start) => route.path(start, finish)?.length - 1);

console.log(allPosibleRoutes.sort()[0]);
